...
class SignTypedDataHelper {
 public:
  enum class Version { kV3, kV4 };
  static std::unique_ptr<SignTypedDataHelper> Create(const base::Value& types,
                                                     Version version);

  ~SignTypedDataHelper();
  SignTypedDataHelper(const SignTypedDataHelper&) = delete;
  SignTypedDataHelper& operator=(const SignTypedDataHelper&) = delete;
 ...
 private:
  ...
  explicit SignTypedDataHelper(const base::Value& types, Version version);
